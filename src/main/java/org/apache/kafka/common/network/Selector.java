...
public class Selector implements Selectable {
    ...
    private final Map<KafkaChannel, org.apache.kafka.common.utils.Deque<NetworkReceive>> stagedReceives;
    ...
    private void clear() {
        ...
        for (Iterator<Map.Entry<String, KafkaChannel>> it = closingChannels.entrySet().iterator(); it.hasNext(); ) {
            ...
            org.apache.kafka.common.utils.Deque<NetworkReceive> deque = this.stagedReceives.get(channel);
    ...
    private void addToStagedReceives(KafkaChannel channel, NetworkReceive receive) {
        if (!stagedReceives.containsKey(channel))
            stagedReceives.put(channel, new org.apache.kafka.common.utils.ArrayDeque<NetworkReceive>());

        org.apache.kafka.common.utils.Deque<NetworkReceive> deque = stagedReceives.get(channel);
        deque.add(receive);
    }
    ...
    private void addToCompletedReceives() {
        if (!this.stagedReceives.isEmpty()) {
            Iterator<Map.Entry<KafkaChannel, org.apache.kafka.common.utils.Deque<NetworkReceive>>> iter = this.stagedReceives.entrySet().iterator();
            while (iter.hasNext()) {
                Map.Entry<KafkaChannel, org.apache.kafka.common.utils.Deque<NetworkReceive>> entry = iter.next();
                KafkaChannel channel = entry.getKey();
                if (!channel.isMute()) {
                    org.apache.kafka.common.utils.Deque<NetworkReceive> deque = entry.getValue();
                    addToCompletedReceives(channel, deque);
                    if (deque.isEmpty())
                        iter.remove();
                }
    ...
    private void addToCompletedReceives(KafkaChannel channel, org.apache.kafka.common.utils.Deque<NetworkReceive> stagedDeque) {
