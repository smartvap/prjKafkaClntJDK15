...
package org.apache.bi_kafka.common.security.kerberos;
...
import org.apache.bi_kafka.common.security.authenticator.AbstractLogin;
import org.apache.bi_kafka.common.utils.Shell;
                    ...
                    if (tgt == null) {
                        ...
                        if (isUsingTicketCache && tgt.getRenewTill() != null && tgt.getRenewTill().getTime() < expiry) {
                            log.warn("The TGT cannot be renewed beyond the next expiry date: {}." +
                                    ...
                                    new Object[]{expiryDate, principal, principal});
                            return;
                        }
                        ...
                        if ((nextRefresh > expiry) || (now + minTimeBeforeRelogin > expiry)) {
                            ...
                            if (nextRefresh < (now + minTimeBeforeRelogin)) {
                                ...
                                log.warn("[Principal={}]: TGT refresh thread time adjusted from {} to {} since the former is sooner " +
                                        ...
                                        new Object[]{principal, until, newUntil, minTimeBeforeRelogin / 1000});
                            }
                        ...
                        if (nextRefresh > expiry) {
                            log.error("[Principal={}]: Next refresh: {} is later than expiry {}. This may indicate a clock skew problem." +
                                    ...
                                    new Object[]{principal, nextRefreshDate, expiryDate});
                            return;
                        }
                    }
                    if (now < nextRefresh) {
                        ...
                        while (retry >= 0) {
                            try {
                                log.debug("[Principal={}]: Running ticket cache refresh command: {} {}",
                                        new Object[]{principal, kinitCmd, kinitArgs});
                                ...
                                if (retry > 0) {
                                    ...
                                    log.warn("[Principal={}]: Could not renew TGT due to problem running shell command: '{} {}'; " +
                                            ...
                                            new Object[]{principal, kinitCmd, kinitArgs, e});
                                    return;
                                }
                            }
                        }
