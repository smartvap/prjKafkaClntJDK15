 ...
 */
package org.apache.bi_kafka.common.network;
...
public class Selector implements Selectable {
    ...
    private final Map<KafkaChannel, org.apache.bi_kafka.common.utils.Deque<NetworkReceive>> stagedReceives;
    ...
    public void connect(String id, InetSocketAddress address, int sendBufferSize, int receiveBufferSize) throws Exception {
        ...
        try {
            ...
            socketChannel.close();
            throw new Exception("Can't resolve address: " + address, e);
        } catch (IOException e) {
            socketChannel.close();
            throw e;
        }
                ...
                if (isImmediatelyConnected || key.isConnectable()) {
                    if (channel.finishConnect()) {
                        ...
                        log.debug("Created socket with SO_RCVBUF = {}, SO_SNDBUF = {}, SO_TIMEOUT = {} to node {}",
                                new Object[]{socketChannel.socket().getReceiveBufferSize(),
    ...
    private void clear() {
        ...
        for (Iterator<Map.Entry<String, KafkaChannel>> it = closingChannels.entrySet().iterator(); it.hasNext(); ) {
            ...
            org.apache.bi_kafka.common.utils.Deque<NetworkReceive> deque = this.stagedReceives.get(channel);
    ...
    private void addToStagedReceives(KafkaChannel channel, NetworkReceive receive) {
        if (!stagedReceives.containsKey(channel))
            stagedReceives.put(channel, new org.apache.bi_kafka.common.utils.ArrayDeque<NetworkReceive>());

        org.apache.bi_kafka.common.utils.Deque<NetworkReceive> deque = stagedReceives.get(channel);
        deque.add(receive);
    }
    ...
    private void addToCompletedReceives() {
        if (!this.stagedReceives.isEmpty()) {
            Iterator<Map.Entry<KafkaChannel, org.apache.bi_kafka.common.utils.Deque<NetworkReceive>>> iter = this.stagedReceives.entrySet().iterator();
            while (iter.hasNext()) {
                Map.Entry<KafkaChannel, org.apache.bi_kafka.common.utils.Deque<NetworkReceive>> entry = iter.next();
                KafkaChannel channel = entry.getKey();
                if (!channel.isMute()) {
                    org.apache.bi_kafka.common.utils.Deque<NetworkReceive> deque = entry.getValue();
                    addToCompletedReceives(channel, deque);
                    if (deque.isEmpty())
                        iter.remove();
                }
    ...
    private void addToCompletedReceives(KafkaChannel channel, org.apache.bi_kafka.common.utils.Deque<NetworkReceive> stagedDeque) {
    ...
    private class SelectorMetrics {
        ...
        public void maybeRegisterConnectionMetrics(String connectionId) {
            if (!"".equals(connectionId) && metricsPerConnection) {
