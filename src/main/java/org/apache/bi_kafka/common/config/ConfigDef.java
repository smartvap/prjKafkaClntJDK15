 ...
 */
package org.apache.bi_kafka.common.config;
...
import org.apache.bi_kafka.common.config.types.Password;
...
public class ConfigDef {
    ...
    protected String getConfigValue(ConfigKey key, String headerName) {
        if ("Name".equals(headerName)) {
            return key.name;
        } else if ("Description".equals(headerName)) {
            return key.documentation;
        } else if ("Type".equals(headerName)) {
            return key.type.toString().toLowerCase();
        } else if ("Default".equals(headerName)) {
            if (key.hasDefault()) {
                if (key.defaultValue == null)
                    return "null";
                String defaultValueStr = convertToString(key.defaultValue, key.type);
                if ("".equals(defaultValueStr))
                    return "\"\"";
                else
                    return defaultValueStr;
            } else
                return "";
        } else if ("Valid Values".equals(headerName)) {
            return key.validator != null ? key.validator.toString() : "";
        } else if ("Importance".equals(headerName)) {
            return key.importance.toString().toLowerCase();
        } else {
        ...
        Collections.sort(configs, new Comparator<ConfigKey>() {
            public int compare(ConfigKey k1, ConfigKey k2) {
                ...
                int cmp = k1.group == null
                        ? (k2.group == null ? 0 : -1)
                        : (k2.group == null ? 1 : (groupOrd.get(k1.group) < groupOrd.get(k2.group) ? -1 : ((groupOrd.get(k1.group) == groupOrd.get(k2.group)) ? 0 : 1)));
                if (cmp == 0) {
                    //k1.orderInGroup, k2.orderInGroup
                    cmp = k1.orderInGroup < k2.orderInGroup ? -1 : ((k1.orderInGroup == k2.orderInGroup) ? 0 : 1);
                    if (cmp == 0) {
