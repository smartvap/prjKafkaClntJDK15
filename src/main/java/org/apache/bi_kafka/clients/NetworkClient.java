...
import org.apache.bi_kafka.common.errors.UnsupportedVersionException;
import org.apache.bi_kafka.common.network.NetworkReceive;
import org.apache.bi_kafka.common.network.Selectable;
import org.apache.bi_kafka.common.network.Send;
import org.apache.bi_kafka.common.protocol.ApiKeys;
import org.apache.bi_kafka.common.protocol.Errors;
import org.apache.bi_kafka.common.protocol.ProtoUtils;
import org.apache.bi_kafka.common.protocol.types.Struct;
...
public class NetworkClient implements KafkaClient {
    ...
    private final Set<String> nodesNeedingApiVersionsFetch = new HashSet<String>();
     ...
     * usually transient and can be resumed in the next {@link #ready(org.apache.bi_kafka.common.Node, long)} }
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now) {
        ...
        try {
            ...
            if (versionInfo == null) {
                if (discoverBrokerVersions && log.isTraceEnabled())
                    log.trace("No version information found when sending message of type {} to node {}. " +
                            "Assuming version {}.", new Object[]{clientRequest.apiKey(), nodeId, builder.version()});
            } else {
            ...
            log.debug("Version mismatch when attempting to send {} to {}",
                    new Object[]{clientRequest.toString(), clientRequest.destination()}, e);
        ...
        if (log.isDebugEnabled()) {
            ...
            if (header.apiVersion() == latestClientVersion) {
                ...
                log.debug("Using older server API v{} to send {} to node {}.",
                        new Object[]{header.apiVersion(), request, nodeId});
            }
        }
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (currInflight == 0 && this.connectionStates.isReady(node.idString())) {
                ...
                log.trace("Removing node {} from least loaded node selection: is-blacked-out: {}, in-flight-requests: {}",
                        new Object[]{node, this.connectionStates.isBlackedOut(node.idString(), now), currInflight});
            }
        }
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            log.trace("Completed receive from node {}, for key {}, received {}",
                    new Object[]{req.destination, req.header.apiKey(), body});
    ...
    private void initiateConnect(Node node, long now) {
        ...
        try {
            log.debug("Initiating connection to node {} at {}:{}.",
                    new Object[]{node.id(), node.host(), node.port()});
            ...
            metadataUpdater.requestUpdate();
            log.debug("Error connecting to node {} at {}:{}:",
                    new Object[]{node.id(), node.host(), node.port()}, e);
        } catch (Exception e) {
            log.warn("Error connecting to node {} at {}:{}:",
                    new Object[]{node.id(), node.host(), node.port()}, e);
        }
    }
        ...
        private long maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId)) {
                ...
                if (metadata.needMetadataForAllTopics())
                    ...
                    metadataRequest = new MetadataRequest.Builder(new ArrayList<String>(metadata.topics()));
