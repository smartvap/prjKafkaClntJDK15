...
import org.apache.bi_kafka.clients.ClientResponse;
import org.apache.bi_kafka.clients.Metadata;
import org.apache.bi_kafka.clients.consumer.*;
import org.apache.bi_kafka.clients.consumer.OffsetOutOfRangeException;
...
import org.apache.bi_kafka.common.metrics.stats.*;
import org.apache.bi_kafka.common.protocol.Errors;
import org.apache.bi_kafka.common.record.InvalidRecordException;
import org.apache.bi_kafka.common.record.LogEntry;
import org.apache.bi_kafka.common.record.Record;
import org.apache.bi_kafka.common.record.TimestampType;
import org.apache.bi_kafka.common.requests.*;
import org.apache.bi_kafka.common.requests.FetchRequest.Builder;
import org.apache.bi_kafka.common.requests.FetchRequest.PartitionData;
...
public class Fetcher<K, V> implements SubscriptionState.Listener {
            ...
            client.send(fetchTarget, request)
                    .addListener(new RequestFutureListener<ClientResponse>() {
                        ...
                        public void onFailure(RuntimeException e) {
                            log.debug("Fetch request to {} for partitions {} failed",
                                    new Object[]{fetchTarget, request.fetchData().keySet()}, e);
                        }
                    });
        }
     ...
     * @throws org.apache.bi_kafka.clients.consumer.NoOffsetForPartitionException If no offset reset strategy is defined
     */
            ...
            if (!subscriptions.isFetchable(partitionRecords.partition)) {
                ...
                if (!partRecords.isEmpty()) {
                    ...
                    log.trace("Returning fetched records at offset {} for assigned partition {} and update " +
                            "position to {}", new Object[]{position, partitionRecords.partition, nextOffset});
                ...
                log.debug("Ignoring fetched records for {} at offset {} since the current position is {}",
                        new Object[]{partitionRecords.partition, partitionRecords.fetchOffset, position});
            }
        }
            ...
            if (error == Errors.NONE) {
                if (partitionData.offsets != null) {
                    ...
                    log.debug("Handling ListOffsetResponse response for {}. Fetched offset {}, timestamp {}",
                            new Object[]{topicPartition, partitionData.offset, partitionData.timestamp});
    ...
    private Map<Node, FetchRequest.Builder> createFetchRequests() {
        ...
        for (TopicPartition partition : fetchablePartitions()) {
            ...
            if (node == null) {
                ...
                log.trace("Added fetch request for partition {} at offset {} to node {}",
                        new Object[]{partition, position, node});
            } else {
            ...
            if (!subscriptions.isFetchable(tp)) {
                ...
                if (position == null || position != fetchOffset) {
                    log.debug("Discarding stale fetch response for partition {} since its offset {} does not match " +
                            "the expected offset {}", new Object[]{tp, fetchOffset, position});
                    return null;
                }
                ...
                if (partition.highWatermark >= 0) {
                    log.trace("Received {} records in fetch response for partition {} with offset {}",
                            new Object[]{parsed.size(), tp, position});
                ...
                if (fetchOffset != subscriptions.position(tp)) {
                    log.debug("Discarding stale fetch response for partition {} since the fetched offset {}" +
                            "does not match the current offset {}",
                            new Object[]{tp, fetchOffset, subscriptions.position(tp)});
